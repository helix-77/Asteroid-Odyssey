...
interface ImpactMapProps {
  ...
  onMapClick: (lat: number, lon: number, terrain: 'land' | 'water') => void;
}
...
export function ImpactMap({
  ...
  useEffect(() => {
    ...
    svg.append('g')
      ...
      .attr('fill', (d: any) => {
        ...
        return d3.color(getCountryFill())?.darker(variation).toString() || getCountryFill();
      })
      .attr('stroke', '#2b8a3e')
    ...
    if (impactLocation && animationProgress > 0) {
      ...
      if (impactCoords) {
        ...
        const kmToPixels = (km: number) => {
          ...
          const point2 = projection([impactLocation.lon + degrees, impactLocation.lat]);
          if (point1 && point2) {
        ...
        if (animationProgress > 0.1) {
          const seismicR = kmToPixels(seismicRadius * animationProgress);
          svg.append('circle')
            .attr('cx', cx)
            .attr('cy', cy)
            .attr('r', seismicR)
        ...
        if (airblastRadii && animationProgress > 0.2) {
          // 1 psi - Glass breakage (light blue)
          const r1psi = kmToPixels(airblastRadii.overpressure_1psi * Math.min(animationProgress * 2, 1));
          svg.append('circle')
            .attr('cx', cx)
            .attr('cy', cy)
            .attr('r', r1psi)
          ...
          if (animationProgress > 0.3) {
            const r5psi = kmToPixels(airblastRadii.overpressure_5psi * Math.min(animationProgress * 2, 1));
            svg.append('circle')
              .attr('cx', cx)
              .attr('cy', cy)
              .attr('r', r5psi)
          ...
          if (animationProgress > 0.4) {
            const r10psi = kmToPixels(airblastRadii.overpressure_10psi * Math.min(animationProgress * 2, 1));
            svg.append('circle')
              .attr('cx', cx)
              .attr('cy', cy)
              .attr('r', r10psi)
          ...
          if (animationProgress > 0.5) {
            const r20psi = kmToPixels(airblastRadii.overpressure_20psi * Math.min(animationProgress * 2, 1));
            svg.append('circle')
              .attr('cx', cx)
              .attr('cy', cy)
              .attr('r', r20psi)
        ...
        // Thermal effects
        if (animationProgress > 0.25 && thermalRadius > 0) {
          const thermalR = kmToPixels(thermalRadius * Math.min(animationProgress * 2, 1));
          svg.append('circle')
            .attr('cx', cx)
            .attr('cy', cy)
            .attr('r', thermalR)
        ...
        if (animationProgress > 0.15) {
          const craterR = kmToPixels(craterDiameter / 2) * Math.min(animationProgress * 5, 1);
        ...
        if (showTsunami && animationProgress > 0.3) {
  ...
  }, [worldData, dimensions, impactLocation, craterDiameter, airblastRadii, thermalRadius, seismicRadius, activeLayer, showTsunami, animationProgress]);
