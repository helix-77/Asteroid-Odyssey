...
const layers = [
  { id: 'population', label: 'Population Density', icon: '👥', color: 'bg-blue-500' },
  { id: 'terrain', label: 'Terrain', icon: '⛰️', color: 'bg-green-500' },
  { id: 'infrastructure', label: 'Infrastructure', icon: '🏗️', color: 'bg-yellow-500' },
];
  ...
  return (
    <Card className="absolute top-4 left-4 p-2 shadow-lg">
      <div className="space-y-1">
        <div className="text-xs font-semibold text-muted-foreground px-2 mb-2">Map Layers</div>
        {layers.map((layer) => (
          <Button
            key={layer.id}
            size="sm"
