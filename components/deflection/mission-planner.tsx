...
import { calculateDeflectionSuccess } from "@/lib/calculations/deflection"
...
export function MissionPlanner({ selectedAsteroid, selectedStrategy, onMissionPlan }: MissionPlannerProps) {
  ...
  const calculateMission = async () => {
    ...
    const plan = calculateDeflectionSuccess(
      selectedAsteroid,
      selectedStrategy,
      launchWindow[0],
      missionDuration[0],
    ...
    <Card className="h-full">
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-4">
          <div>
            <label className="text-sm font-medium">Launch Window: {launchWindow[0]} years from now</label>
            <Slider value={launchWindow} onValueChange={setLaunchWindow} max={10} min={1} step={0.5} className="mt-2" />
            <div className="text-xs text-muted-foreground mt-1">
              Earlier launches have higher success rates but less preparation time
            </div>
          </div>

          <div>
            <label className="text-sm font-medium">Mission Duration: {missionDuration[0]} years</label>
            <Slider
            ...
            />
            <div className="text-xs text-muted-foreground mt-1">
              Longer missions allow for more precise targeting and course corrections
            </div>
          </div>

          <div>
            <label className="text-sm font-medium">Number of Missions: {numberOfMissions[0]}</label>
            <Slider
        ...
        {/* Calculate Button */}
        <Button onClick={calculateMission} disabled={isCalculating} className="w-full">
        ...
        {isCalculating && (
          <div className="space-y-2">
            <div className="text-sm text-muted-foreground">Analyzing orbital mechanics...</div>
        ...
        {missionPlan && (
          <div className="space-y-4 border-t pt-4">
            <div className="flex items-center justify-between">
              ...
              <Badge className={getRiskLevel(missionPlan.successProbability).color}>
            ...
            <div className="bg-blue-50 p-4 rounded-lg">
              ...
              <div className={`text-3xl font-bold ${getSuccessColor(missionPlan.successProbability)}`}>
                {(missionPlan.successProbability * 100).toFixed(1)}%
              </div>
              <Progress value={missionPlan.successProbability * 100} className="mt-2" />
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <div className="font-semibold">${missionPlan.totalCost.toLocaleString()}M</div>
              </div>

              <div className="space-y-2">
                <div className="text-sm text-muted-foreground">Timeline</div>
                <div className="font-semibold">{missionPlan.totalTime} years</div>
              </div>

              <div className="space-y-2">
                <div className="text-sm text-muted-foreground">Deflection Distance</div>
                <div className="font-semibold">{missionPlan.deflectionDistance.toLocaleString()} km</div>
              </div>

              <div className="space-y-2">
                <div className="text-sm text-muted-foreground">Delta-V Required</div>
                <div className="font-semibold">{missionPlan.deltaVRequired.toFixed(2)} m/s</div>
              </div>
            </div>
            ...
            {missionPlan.risks && missionPlan.risks.length > 0 && (
              <div className="space-y-2">
                ...
                <div className="space-y-1">
                  {missionPlan.risks.slice(0, 3).map((risk: string, index: number) => (
