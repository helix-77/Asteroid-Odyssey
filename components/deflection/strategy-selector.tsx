...
import { Rocket, Zap, Magnet, Sun, AlertTriangle, Clock, DollarSign } from "lucide-react"
...
export function StrategySelector({ selectedAsteroid, onStrategySelect, selectedStrategy }: StrategySelectorProps) {
  ...
  if (!selectedAsteroid) {
    return (
      <Card className="h-full">
        ...
        <CardContent>
          <div className="text-center text-muted-foreground py-8">Select an asteroid to view deflection options</div>
        </CardContent>
      </Card>
    )
  }
  ...
  return (
    <Card className="h-full">
      <CardHeader>
        ...
        <p className="text-sm text-muted-foreground">Choose a method to deflect {selectedAsteroid.name}</p>
      </CardHeader>
      <CardContent>
                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-3">
                    <div>
                      ...
                      <div className={`font-semibold ${getEffectivenessColor(strategy.effectiveness)}`}>
                        {(strategy.effectiveness * 100).toFixed(0)}%
                      </div>
                      <Progress value={strategy.effectiveness * 100} className="h-1 mt-1" />
                    </div>
                  ...
                  {strategy.risks.length > 0 && (
                    <div className="mt-3 pt-3 border-t">
                      ...
                      <div className="text-xs text-red-600">{strategy.risks.slice(0, 2).join(", ")}</div>
                    </div>
                  )}
                </div>
              ))}
