...
export default function AdvancedEarth3D({
  ...
  const [asteroidPositions, setAsteroidPositions] = useState<Map<string, Vector3>>(new Map())
  const [impactPoint, setImpactPoint] = useState<Vector3 | null>(null)
  const [cameraMode, setCameraMode] = useState<"free" | "earth" | "asteroid" | "impact">("earth")

  // Update asteroid positions based on orbital mechanics
  useEffect(() => {
    const updatePositions = () => {
      const currentJD = 2460000 + ((Date.now() - Date.now()) / (1000 * 60 * 60 * 24)) * timeScale
      ...
      asteroidData.asteroids.forEach((asteroid) => {
        ...
        const position = new Vector3(state.position.x * 2, state.position.y * 2, state.position.z * 2)
  ...
  useEffect(() => {
    switch (simulationMode) {
      ...
      case "impact":
        ...
        if (selectedAsteroid) {
          const scenario = impactData.scenarios.find((s) => s.asteroid_id === selectedAsteroid)
          if (scenario) {
  ...
  const selectedAsteroidData = asteroidData.asteroids.find((a) => a.id === selectedAsteroid)
  const selectedPosition = selectedAsteroid ? asteroidPositions.get(selectedAsteroid) : undefined
  const selectedImpactScenario = impactData.scenarios.find((s) => s.asteroid_id === selectedAsteroid)
    ...
    <div className="w-full h-full">
      <Canvas camera={{ position: [0, 2, 8], fov: 45 }} gl={{ antialias: true, alpha: true }}>
        <Suspense fallback={null}>
          ...
          {/* Enhanced Earth */}
          <EnhancedEarth showClouds={true} showAtmosphere={true} rotationSpeed={0.001 * timeScale} />
            ...
            return (
              <group key={asteroid.id}>
                <Asteroid
                  ...
                  size={asteroid.size}
                  threatLevel={asteroid.threat_level as "low" | "medium" | "high"}
                ...
                {showOrbits && (
                  <OrbitPath
                    ...
                    color={asteroid.threat_level === "high" ? "#EF4444" : "#F97316"}
                    opacity={selectedAsteroid === asteroid.id ? 0.8 : 0.3}
                    visible={showOrbits}
                  />
                )}
              </group>
          ...
          {simulationMode === "impact" && impactPoint && selectedImpactScenario && (
            <ImpactVisualization
              impactPoint={[impactPoint.x, impactPoint.y, impactPoint.z]}
              craterDiameter={selectedImpactScenario.impact.crater.diameter / 1000} // Convert to km
              airblastRadius={selectedImpactScenario.impact.effects.airblast_radius}
              thermalRadius={selectedImpactScenario.impact.effects.thermal_radiation}
              active={true}
            />
          )}
