...
const REGIONS = {
  global: { center: [0, 0], scale: 1 },
  asia: { center: [100, 30], scale: 3 },
  europe: { center: [10, 50], scale: 4 },
  americas: { center: [-90, 0], scale: 2.5 },
  africa: { center: [20, 0], scale: 3 },
  australia: { center: [135, -25], scale: 5 },
};
...
export function ImpactMap({
  ...
  useEffect(() => {
    ...
    // Create projection with region focus
    const region =
      REGIONS[selectedRegion as keyof typeof REGIONS] || REGIONS.global;
    const projection = d3
      .geoNaturalEarth1()
      .scale((width / 5.5) * region.scale)
    ...
    if (impactLocation && animationProgress > 0) {
      ...
      if (impactCoords) {
        ...
        if (animationProgress > 0.1 && impactResults) {
          const seismicRadius =
            impactResults.blastRadiiKm?.overpressure1psi?.value * 2 || 50;
        ...
        if (impactResults?.blastRadiiKm && animationProgress > 0.2) {
          // 1 psi - Glass breakage (light blue)
          const r1psi = kmToPixels(
            impactResults.blastRadiiKm.overpressure1psi.value *
              Math.min(animationProgress * 2, 1)
          );
          svg
            .append("circle")
            .attr("cx", cx)
        ...
        if (animationProgress > 0.15 && impactResults?.craterRadiusKm) {
          const craterR =
            kmToPixels(impactResults.craterRadiusKm.value / 2) *
            Math.min(animationProgress * 5, 1);
